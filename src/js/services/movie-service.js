...
class MovieService {
	...
	getMoviesbyPopularity() {
		return new Promise((resolve, reject) => {
			let request = new XMLHttpRequest();
			request.open("GET", `https://api.themoviedb.org/3/discover/movie?api_key=33e38078c04b3d1e0b3d69f5e283a1f4&sort_by=popularity.desc`, true);
			request.onload = function () {
				resolve(JSON.parse(this.response))
				// console.log(JSON.parse(this.response))
			};
	...
	getTrending(type,validTime) {
		return new Promise((resolve, reject) => {
			let request = new XMLHttpRequest();
			request.open("GET", `https://api.themoviedb.org/3/trending/${type}/${validTime}?api_key=33e38078c04b3d1e0b3d69f5e283a1f4`, true);
			request.onload = function () {
				resolve(JSON.parse(this.response))
				// console.log(JSON.parse(this.response))
			};
